...
export async function escribirReporteDeCatalogo(filas: readonly FilaDeCatalogo[]): Promise<{ json: string; md: string }> {
  ...
  const md = [
    ...
    '',
    'Este reporte sólo MAPEA el comportamiento real y la seguridad (no hay crash / pantalla',
    'blanca al elegir cada opción). No decide qué artefacto *debería* preguntar: eso es',
    'materia de una corrida posterior (CAT-CONN-01).',
    '',
    encabezado,
    separador,
    ...cuerpo,
