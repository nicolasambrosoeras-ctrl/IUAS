...
// cota de piso de su Nivel (FIX-M1-MULTINIVEL-BASE-LEVEL-01), el artefacto muestra su altura sugerida y su
...
test.describe('GEOM-UX-01 · cotas hidráulicas heredadas (M1)', () => {
  test('herencia UF→Local, sugerida, cota efectiva, personalizar/restablecer y cambio de tipo', async ({
    page,
    baseURLEfectiva,
  }) => {
    ...
    // Primer artefacto del Baño = Lavatorio: altura sugerida 0,90 y
    ...
    await expect(cotaLavatorio).toContainText('0,90 m · sugerida')
    await expect(cotaLavatorio).toContainText('Cota hidráulica efectiva: +0,90 m')

    // Segundo artefacto = Ducha: sugerida 2,00 -> efectiva +2,00 m.
    const ducha = bano.locator('.m1-artefacto').nth(1)
    const cotaDucha = ducha.locator('.m1-cota')
    await expect(cotaDucha).toContainText('2,00 m · sugerida')
    ...
    await expect(cotaDucha).toContainText('Cota hidráulica efectiva: +2,00 m')
    await expect(cotaDucha).toContainText('sugerida')

    // --- Restablecer la altura del lavatorio -> vuelve a la sugerida ---
    ...
    // adoptar el valor por defecto del bidet (0,40), no arrastrar 2,20.
    ...
    await estabilizar(page)
    await expect(cotaDucha).toContainText('0,40 m · sugerida')
